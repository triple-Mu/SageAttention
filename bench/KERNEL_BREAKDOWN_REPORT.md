...
**核心产物已入库**:`bench/kernel_breakdown_data/` 存有 8 份原始采集 JSON(gzip,解压后 SHA256 与下表逐一对应)与 4 份合并 CSV——报告里每个数字都能回溯到这些文件。日志与 smoke 等外围产物不入库,索引如下,SHA256 取前 8 位用于核对手上的文件是否是本报告所依据的那一份。
