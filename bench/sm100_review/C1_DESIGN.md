...
### 16.3 wave27:L-b1 上机实测——bench 与 ncu 双闸不达线,判负 revert;尾波假设证伪

**裁决:revert**(实现 5bc0be0,revert 2f54e29,树回 f141046 字节同)。
预注册判据三条全部不达线;§16.2 的「尾波 heavy-last 可修」假设本身被
测量证伪(下详)。

**实现与本地闸(全过;实现细节看 5bc0be0 的 commit body)**:kernel 内
`cta_q() = gridDim.x-1-blockIdx.x`(causal constexpr 门控,非 causal
恒等映射),四个使用点(trip_count / softmax cta_idx_q / load warp /
epi q_base)统一走 cta_q()。40-reg mma 区寄存器纪律花了三轮迭代:
volatile pin 逐 call site 钉住翻转值、trip1 下沉到 steady loop 之后
(volatile asm 源序即执行序,提前算就要跨 loop 携带 = 4B spill)、
S1-drain 与 steady loop 共用计数器且 constexpr 门到 causal(合并计数器
会破坏 nvcc 对 kNone 空循环的证明)。本地闸:ptxas 16/16 = 128 entry /
0 spill / 0 stack、无 C7508、USETMAXREG 0xc0/0x58/0x28 各 ×4;FP opcode
逐类对账(全 TU 33 kernel,全函数 + 128-MUFU 回跳段双口径)全等;
sass_diff 全仓 433 kernel:diff 仅 ws TU 的 16 个 causal 实例,16 个
kNone 实例逐指令同,其余 TU 逐字节同。

**B200 会话**(tree=sage-w15=5bc0be0,基线臂 sage-w14;JID 4040831,
umb-b200-248 与 w26 同节点,健康门过,单 alloc 34 分钟完毕即 cancel):
golden 三轨 diff=0(ws0 ok=2082 / ws1 ok=2107 / wsp ok=2107),routing
4/4,stress auto_d128 / auto_d64 / 强制 per-tile(sweep+2000 定点)全绿。
新增的 W=1 b1h1 退化面(s256-1024,randn scale)暴露强制 per-tile c0
输出非有限——kNone 实例与基线二进制同,A/B 三臂归类见节末。

**bench d128 22 点**(自由时钟,new/base/cudnn ×3 轮 median,全 excl;
`logs-w27/bench-d128/`):

| shape | new TF | base TF | new/base | new/cudnn | base/cudnn |
|---|---:|---:|---:|---:|---:|
| c0 b1 s1024 | 738.0 | 737.8 | 1.0002 | 1.0712 | 1.0710 |
| c0 b1 s4096 | 1118.3 | 1113.3 | 1.0046 | 0.8793 | 0.8753 |
| c0 b1 s16384 | 1374.4 | 1374.8 | 0.9997 | 1.0158 | 1.0161 |
| c0 b1 s32768 | 1394.0 | 1393.9 | 1.0001 | 1.0683 | 1.0682 |
| c0 b1 s131072 | 1425.7 | 1425.1 | 1.0005 | 1.1123 | 1.1118 |
| c0 b4 s1024 | 934.2 | 933.8 | 1.0005 | 0.8663 | 0.8659 |
| c0 b4 s4096 | 1295.2 | 1296.7 | 0.9988 | 0.9484 | 0.9495 |
| c0 b4 s16384 | 1379.9 | 1381.0 | 0.9992 | 1.0708 | 1.0717 |
| c0 b4 s32768 | 1405.7 | 1405.8 | 1.0000 | 1.0992 | 1.0993 |
| c0 b4 s131072 | 1428.3 | 1428.3 | 1.0000 | 1.1230 | 1.1230 |
| c1 b1 s1024 | 362.1 | 375.1 | **0.9653** | 0.8849 | 0.9167 |
| c1 b1 s4096 | 789.1 | 793.0 | **0.9951** | 0.6837 | 0.6871 |
| c1 b1 s16384 | 1199.0 | 1186.8 | 1.0103 | 0.8950 | 0.8858 |
| c1 b1 s32768 | 1309.7 | 1292.4 | 1.0133 | 1.0209 | 1.0075 |
| c1 b1 s131072 | 1396.2 | 1385.4 | 1.0078 | 1.1152 | 1.1066 |
| c1 b4 s1024 | 468.0 | 474.4 | **0.9865** | 0.6439 | 0.6527 |
| c1 b4 s4096 | 957.2 | 975.2 | **0.9815** | 0.8184 | 0.8338 |
| c1 b4 s16384 | 1256.3 | 1264.5 | **0.9935** | 1.0046 | 1.0112 |
| c1 b4 s32768 | 1329.3 | 1333.3 | 0.9970 | 1.0826 | 1.0858 |
| c1 b4 s131072 | 1397.0 | 1394.4 | 1.0018 | 1.1122 | 1.1102 |
| d64 c0 b4 s4096(probe) | 627.3 | 627.2 | 1.0002 | 0.6671 | 0.6670 |
| d64 c0 b4 s16384(probe) | 651.1 | 651.1 | 1.0001 | 0.7509 | 0.7509 |

c1 d128 逐 seq geomean(new/base):s1024 0.9758、s4096 **0.9883**
(判据 ≥1.03,FAIL)、s16384 1.0019(判据 ≥1.01,FAIL)、s32768
1.0051、s131072 1.0048;c0 geomean 1.0003(达标)。强制 per-tile c0
spot 三点(d128 s4096/s16384、d64 s16384)0.9994-1.0003(达标,
kNone 二进制同的实机自证;`logs-w27/bench-spot/`)。

**bench d64 12 点**(`logs-w27/bench-d64/`):c0 六点 0.9999-1.0003
(geomean 1.0001,达标);**c1 六点全正**:b1 s4096/16384/32768 =
1.0153/1.0206/1.0215,b4 = 1.0063/1.0069/1.0066,geomean **1.0128**
(spread ≤0.2%)。

**ncu 尾波**(锁基频,b4h32 d128 c1,WS=1 P=0,3 发;
`logs-w27/ncu/bal_*`):

| 指标 | base s4096 | new s4096 | base s16384 | new s16384 |
|---|---:|---:|---:|---:|
| SM active max/avg(中位) | 1.1007 | **1.0988** | 1.0418 | 1.0438 |
| duration(中位) | 939.1 us | **952.4 us** | 11.738 ms | 11.764 ms |

判据 max/avg ≤1.05:FAIL(1.099 纹丝不动);锁频 duration 反而
+1.4%/+0.2%,与自由钟 bench 同向。

**机制读数(三条)**:

1. **尾波假设证伪**:发射序静态翻转对 max/avg 零作用——w26 读到的
   1.098 残差不是「重 CTA 落尾波」的发射序问题。grid x 只有 16 宽、
   每 SM 平均 13.8 个 CTA,per-SM 工作量方差 + 硬件分配器动态回填
   决定不均;静态序无从改变。cudnn 的 1.038 得自 1-D grid + kernel 内
   映射的整套结构,不是一个倒序能取到的。
2. **翻转本身带净损**:b1 s1024 是单波网格(128 CTA < 148 SM),
   尾波机制根本无从参与,却损 3.5%——heavy-first 的损耗另有来源
   (方向:重 CTA 同相扎堆,K/V TMA 供给与 L2 复用相位变差;判负
   即止,未展开归因)。
3. **d64 c1 全线意外正**(+0.6~2.2%,6/6 点):d64 是 XU-bound 形态、
   K/V 字节流减半,同一供给相位机制方向相反受益。**记 d64-only 门控
   翻转为候选 lever**(wave24 vec_full d64-only 复活先例),按
   wave23b 单改动复验条款需独立立项,本轮不实现。

**degen 归类(A/B 三臂,3 seed × 6 shape,`logs-w27/ab_run.log`)**:
new 与 base 18/18 全 NaN 且逐 case 完全同(同 seed 同 n 同行区间——
二进制同的 kNone/causal 实例自证),WS=0 classic 臂 0/18 全有限。
**结论:pre-existing 的 per-tile ws 行为,与 L-b1 无关**。触发条件是
randn 刻度(负/零 q_scale/k_scale,越出量化管线「scale ≥ 0」契约;
golden 语料在契约内所以三轨全绿),b4h32 常规压测面也不触发。产品面
无影响,记独立卫生项待查(ws 路对刻度符号的假设点位),不阻塞本轮
裁决。

数据:`logs-w27/{g_*,routing.log,stress_*,bench-d128,bench-d64,bench-spot,ncu}`、
脚本 `scripts-w27/`(采集 `w27_all.sh`/`w27_orch.sh`,spot/degen 驱动,
分析本地 `analyze_w27.py`)。
