...
>
> wave25 更新:auto 已放宽到 d64 并通过组合态复验 **KEEP**(§8.6)——auto
...
launcher(keep 判决限于 vec_full 单改动)。**→ wave25 已兑现并 KEEP
(§8.6)。**
...
### 8.6 wave25:auto 放宽到 d64(§8.5.3 建议兑现)与组合态复验:**KEEP**(2b652df)

分支 wave25/d64-auto,基线 41e99fe(wave24 KEEP 树收编后的 feat/varlen
tip)。改动只有一处 host 代码(2b652df):`sm100_ws_auto_pick` 去掉
`head_dim == 128` 限制,d64 也进 ws 路;persist 三态不动(kAuto:
nc→persistent、causal→per-tile ws)——d64 的 auto 组合因此正是 §8.5.3 里
12/12 全 ≥1.0319 的形态。dim!=4 防御保留;launcher 注释按 §1.3/§8.5 改写
(「384/512 TMEM 列」归因废弃)。

#### 8.6.1 本地门禁(host-only 改动,应全同,实测全同)

| 项 | 结果 |
|---|---|
| ptxas,两 TU probe 各 4 实例 × sm_100a/sm_110a | 16/16:0 spill / 0 stack / 128 entry;USETMAXREG 0xc0/0x58/0x28 各 ×4(与 §8.5.1 逐项同;probe TU 不含 host 代码,构造性不受本改动影响) |
| 被改 TU(qk_int_sv_f8_cuda_sm100.cu,10.0a)device SASS | 对基线 41e99fe 全文件 cuobjdump diff 空(215568 行),host-only 得证 |

#### 8.6.2 上机门禁(B200,JID 4039021,umbriel-b200-018,健康门一次过)

树 sage-w14.tar.gz = 2b652df,pytorch_26.07 容器,10.0a + PRUNE=OFF,单
alloc 完毕即 cancel。

1. **golden 三轨全 diff=0**(显式 WS=0 / WS=1 / WS=1+PERSIST=1;ws0
   ok=2082 skipped=198、ws1/wsp 各 ok=2107,与 §8.5.3 逐项同)。auto 语义
   变化不影响强制态;auto 态不做 golden——它只换「选哪只已验 kernel」,
   正确性由强制轨覆盖,路由本身走自检。
2. **auto 路由自检 4/4**(torch.profiler 认 kernel 名,`scripts-w25/
   w25_routing.py`):hd64 nc→persist、hd64 c→ws、hd128 nc→persist、
   hd128 c→ws。
3. **压测 3/3 零挂死**:auto 态 d64 与 d128 各全量(SWEEP 2k..128k ×
   causal ×10 + s32768 8000×2,nc 腿即走 persist、causal 腿走 ws);
   persist 强制 d64 定点 2000×2 快速回归(全量面 §8.5.3 已盖)。

#### 8.6.3 bench 组合态终表 = auto/cudnn 全段战线图(median of 3,全部 exclusive)

12 点 d64 + 22 点 d128,auto(WS/PERSIST 全 unset)/ old(WS=0)/ cudnn
×3 轮。参照 = 该形状 auto 实际路由到的强制 arm 在参照会话的读数(c0 →
wsp/old、c1 → ws/old;d64 用 w24、d128 用 w20)。

d64(b·h32,TF = TFLOPS):

| c1 b1 s4096 | 790.6 | 661.5 | 1176.6 | 1.1952 | 1.1929 | 0.6719 |
| c1 b1 s16384 | 1186.3 | 964.6 | 1380.6 | 1.2298 | 1.2301 | 0.8593 |
| c1 b1 s32768 | 1292.2 | 1040.4 | 1296.5 | 1.2421 | 1.2410 | 0.9967 |
| c1 b1 s131072 | 1385.1 | 1103.4 | 1269.3 | 1.2553 | 1.2550 | 1.0912 |
| c1 b4 s1024 | 473.0 | 392.1 | 731.0 | 1.2063 | 1.2072 | 0.6471 |
| c1 b4 s4096 | 976.0 | 798.6 | 1194.4 | 1.2220 | 1.2193 | 0.8171 |
| c1 b4 s16384 | 1264.5 | 1023.8 | 1278.5 | 1.2351 | 1.2347 | 0.9891 |
| c1 b4 s32768 | 1332.8 | 1071.9 | 1239.5 | 1.2434 | 1.2433 | 1.0752 |
| c1 b4 s131072 | 1394.7 | 1110.6 | 1270.2 | 1.2558 | 1.2559 | 1.0980 |

汇总与判据:

| 段 | auto/old geomean | 判据/参照 | auto/cudnn geomean |
|---|---:|---|---:|
| d64 12 点 | **1.0590**(c0 1.0795 / c1 1.0389) | ≥1.03 带:12/12 ≥1.0304,过(w24 组合参照 1.0593,逐形状 max dev 0.35pp) | 0.6769(c0 0.7112 / c1 0.6442) |
| d128 20 点 | **1.2863**(c0 1.3332 / c1 1.2410) | 与 w20/23 参照带逐点吻合(max dev 0.35pp)——auto 语义没动 d128,运行时坐实 | 0.9500(c0 1.0005 / c1 0.9020) |

- 22 点 d128 网格里的两个 d64 nc probe(b4 s4096/s16384)按设计从 classic
  翻到 persist:auto/old 1.1157/1.0738(w20 classic 年代 0.9994/0.9607),
  与 12 点网格同形状读数(1.1148/1.0739)互证。
- 轮间 spread:auto ≤0.20%(d64)/ ≤1.71%(d128,s1024);old ≤0.54%;
  cudnn 短序列 causal 有单轮掉速(c1 b1 s1024 三轮 402/401/290,spread
  28%),median 口径吸收。
- old 列跨会话:d64 对 w24 geomean 0.9997(max dev 0.12%),d128 对 w20
  geomean 0.9993(max dev 0.30%)——节点可比性成立。

**判决:KEEP(VERDICT_D64_AUTO_OK)**。§8.5.3 的 auto 建议(M2)由本节
兑现:sm100 fp8 路径上 auto 已是 d64+d128 全段最优组合,d64 用户侧净收益
+3.0~11.5%(vs classic,零 env 配置)。cudnn 差距的下一战场不变:d64 追平
线受 XU roofline 束缚(§1.2),d128 c1 短序列(s1024-4096 的 0.65-0.82)
是 §7.7 s4096 谷地立项的范围。

数据:集群 `SageAttention_refactor/logs-w25/`(g_*.txt、routing.log、
stress_*.log、bench-d64/、bench-d128/、build.log、clocks.csv)、脚本
