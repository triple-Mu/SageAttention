...
//
// A third and fourth compilation define SAGE_VARLEN: q/k/v are packed to
// [total_tokens, heads, head_dim] and blockIdx.z selects a sequence of a
// cu_seqlens prefix sum instead of a batch entry. Those translation units get
// SAGEATTN_ARCH_NS=sm89_varlen / sm120_varlen from CMake, for the same ODR
// reason (a different kernel body under the same template name). The choice of
// #ifdef over a `bool kVarlen` template parameter is what keeps the dense
// kernels' SASS byte-identical: a template parameter changes the kernel's
// parameter space and its constant-bank layout, so the dense instantiation
// could not be unchanged even where the code is.
...
#ifdef SAGE_VARLEN
                                         // [batch_size + 1] prefix sums; the sequence lengths, the
                                         // scale block bases and the padded V^T slab base are derived
                                         // from them per block. The *_stride_h are the packed tensors'
                                         // per-head extents, which the dense kernel instead reads back
                                         // out of gridDim.
                                         ...
                                         float          sm_scale)
{
...
#ifdef SAGE_VARLEN
    // The packed path forces smooth_v off (plan.cpp): a per-sequence V mean is
    // not the tensor the dense kernels fuse, and the empty-KV shortcut below
    // writes a zero output row, which +V_mean would silently turn into V_mean.
    static_assert(!fuse_v_mean, "the varlen kernels do not implement smooth_v");
#endif
    ...
    // dequant_scale chain, a measured ~1.6% on the L20 - so the plain
    // non-causal instances fold it into the pointer below.
    int64_t      q_scale_idx;
    const float* K_scale_base_ptr;
    uint32_t     k_scale_off;

    // The dense scale layout is [batch, heads, blocks] with the per-head
    // extent read back out of gridDim. Both premises fail in the packed
    // layout, where the grid covers max_seqlen and the earlier sequences' own
    // blocks sit in front of this one's, so the varlen form below replaces
    // this block wholesale rather than adjusting it.
#ifndef SAGE_VARLEN
    ...
    // same expression the quantization kernel wrote them with.
...
#pragma unroll
...
#ifdef SAGE_VARLEN
    // Q and K move by the sequence's first *token*; V moves by the first slot
    // of its *padded slab*. That is the one place the packed fp8 path needs
    // both of varlen.h's coordinate systems at once: V^T is zero-padded per
    // sequence to CTA_K, because the load below has no bound predicate and its
    // 16-token permute groups must land inside one sequence. The host pins
    // v_pad_multiple == CTA_K, which is what makes blk_k_base * CTA_K the
    // slab base (pad_offset).
    ...
    const int8_t* V_lane_base_ptr =
        V + static_cast<int64_t>(seq_info.blk_k_base) * CTA_K
...
#pragma unroll
    ...
    // instances only: their KV loop is almost all kBulk tiles, where the extra
    // S2R sits on the per-iteration dequant_scale chain. The causal and
    // return_lse instances keep the split form - folding it there reshuffles
    // their register allocation for the worse (measured: causal gives half its
    // win back, non-causal+lse spills 39 -> 90).
...
#pragma unroll
        ...
        // V_SMEM_STRIDE / PACK_SIZE_V, 4, use_inst_buf>(
...
#ifdef SAGE_VARLEN
    DTypeOut* O_lane_ptr =
        O + static_cast<int64_t>(seq_info.offset_q) * stride_seq_o + static_cast<int64_t>(head_id) * stride_h_o
