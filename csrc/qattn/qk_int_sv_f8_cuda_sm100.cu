...
template <uint32_t CTA_Q, uint32_t CTA_K, uint32_t NUM_THREADS, uint32_t head_dim,
          QuantGranularity Q_GRAN, QuantGranularity K_GRAN, typename DTypeOut,
          MaskMode mask_mode = MaskMode::kNone, bool return_lse = false,
          bool fuse_v_scale = false, bool PV_FROM_SMEM = false>
__global__ void __launch_bounds__(NUM_THREADS) qk_int8_sv_f8_attn_kernel_sm100(
    ...
    DTypeOut *O, float *__restrict__ Lse,
    ...
    float sm_scale)
{
  ...
  static_assert(TMEM_COL_P >= TMEM_COL_S + TMEM_COLS_S &&
                TMEM_COL_O >= TMEM_COL_P + TMEM_COLS_P, "TMEM regions must not overlap");
  ...
  constexpr tcgen05::SmemSwizzleMode QK_SWIZZLE = (head_dim == 128)
  ...
  constexpr uint32_t idesc_qk =
      tcgen05::make_instr_desc<CTA_Q, CTA_K, tcgen05::kMmaFmtS8, tcgen05::kMmaFmtS8,
                               tcgen05::kMmaCFmtS32, /*AMajorK=*/true, /*BMajorK=*/true>();
  constexpr uint32_t idesc_pv =
      tcgen05::make_instr_desc<CTA_Q, head_dim, tcgen05::kMmaFmtE4M3, tcgen05::kMmaFmtE4M3,
  ...
  else if constexpr (Q_GRAN == QuantGranularity::kPerWarp)
  {
    ...
    q_scale_idx = batch_id * num_qo_heads * num_warp_block_q + head_id * num_warp_block_q +
                  bx * (CTA_Q / 32) + row / 32;
  }
  else if constexpr (Q_GRAN == QuantGranularity::kPerThread)
  {
    ...
    q_scale_idx = batch_id * num_qo_heads * (num_warp_block_q * 8) +
                  head_id * (num_warp_block_q * 8) + bx * ((CTA_Q / 32) * 8) +
                  (row / 32) * 8 + (row % 8);
  }
  ...
  else if constexpr (K_GRAN == QuantGranularity::kPerThread)
  {
    ...
    const uint32_t num_block_k = div_ceil(kv_len, CTA_K);
    k_scale_idx = batch_id * (num_qo_heads / num_kv_groups) * (num_block_k * 4) +
                  (head_id / num_kv_groups) * (num_block_k * 4);
  }
...
#pragma unroll
    ...
    if constexpr (!is_last) {
      ...
      {
        tcgen05::expect_bytes<(CTA_K * head_dim) * sizeof(int8_t)>(&barrier_K);
        tcgen05::load_async_4D(sK, &tensorMapK, &barrier_K, 0, (iter + 1) * CTA_K, kv_head_id,
                               batch_id);
      }
    }
...
#pragma unroll
      for (uint32_t cls = 0; cls < 4; cls++) {
        dq[cls] = q_scale * K_scale[k_scale_idx + iter * k_scale_advance_offset + cls];
      }
    } else {
      dq[0] = q_scale * K_scale[k_scale_idx + iter * k_scale_advance_offset];
    }
...
#pragma unroll
    ...
    if constexpr (!is_last) {
      ...
      {
        tcgen05::expect_bytes<(CTA_K * head_dim) * sizeof(int8_t)>(&barrier_V);
        tcgen05::load_async_4D(sV, &tensorMapV, &barrier_V, (iter + 1) * CTA_K, 0, kv_head_id,
                               batch_id);
      }
    }
  };
  ...
  DTypeOut *O_row_ptr = O + batch_id * stride_bz_o + head_id * stride_h_o + q_idx * stride_seq_o;
  const float *V_scale_ptr =
      fuse_v_scale ? V_scale + batch_id * (num_qo_heads / num_kv_groups) * head_dim +
                         kv_head_id * head_dim
                   : nullptr;
...
#pragma unroll
  ...
  if constexpr (return_lse)
  {
    ...
    if (row_valid)
    {
      Lse[batch_id * (qo_len * num_qo_heads) + head_id * qo_len + q_idx] =
          math::ptx_log2(d) + m;
    }
  }
...
torch::Tensor qk_int8_sv_f8_accum_f32_attn(
                  torch::Tensor query,
                  torch::Tensor key,
                  torch::Tensor value,
                  torch::Tensor output,
                  ...
                  int return_lse)
{
  QKVLayout qkv = qkv_layout_parse<QKVFamily::kSVF8TMA>(
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_type, DTypeOut, {
            ...
            {
              CHECK_SHAPE(query_scale, batch_size, num_qo_heads, static_cast<long>(div_ceil(qo_len, CTA_Q) * (CTA_Q / 32)));
            ...
            {
              CHECK_SHAPE(query_scale, batch_size, num_qo_heads, static_cast<long>(div_ceil(qo_len, CTA_Q) * (CTA_Q / 32) * 8));
            ...
            auto* kernel = qk_int8_sv_f8_attn_kernel_sm100<CTA_Q, CTA_K, NUM_THREADS, HEAD_DIM, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), DTypeOut, mask_mode, RETURN_LSE, false, kPVFromSmem>;
            size_t sMemSize = CTA_Q * HEAD_DIM * sizeof(int8_t) + CTA_K * HEAD_DIM * sizeof(int8_t) + CTA_K * HEAD_DIM * sizeof(int8_t) + (kPVFromSmem ? CTA_Q * CTA_K * sizeof(int8_t) : 0);
...
torch::Tensor qk_int8_sv_f8_accum_f32_fuse_v_scale_attn(
                    torch::Tensor query,
                    torch::Tensor key,
                    torch::Tensor value,
                    torch::Tensor output,
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            auto* kernel = qk_int8_sv_f8_attn_kernel_sm100<CTA_Q, CTA_K, NUM_THREADS, HEAD_DIM, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), DTypeOut, mask_mode, RETURN_LSE, true, kPVFromSmem>;
            size_t sMemSize = CTA_Q * HEAD_DIM * sizeof(int8_t) + CTA_K * HEAD_DIM * sizeof(int8_t) + CTA_K * HEAD_DIM * sizeof(int8_t) + (kPVFromSmem ? CTA_Q * CTA_K * sizeof(int8_t) : 0);
            cudaFuncSetAttribute(
                kernel,
                cudaFuncAttributeMaxDynamicSharedMemorySize, sMemSize);
