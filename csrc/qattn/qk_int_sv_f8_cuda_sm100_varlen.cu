...
// The varlen half of the sm100 attention kernel: the same body as
// qk_int_sv_f8_cuda_sm100.cu, compiled from the same header with SAGE_VARLEN
...
//
// One variant is instantiated, the one plan.cpp resolves to for sm100: fused V
// scale, true fp32 accumulation. The warp-specialized kernel has no varlen
// path yet, so the SAGEATTN_SM100_WS routing of the dense launcher is not
// replicated here - varlen always runs the classic 128-thread kernel.
...
#include "attn_cuda_sm100_varlen.h"
...
torch::Tensor qk_int8_sv_f8_accum_f32_fuse_v_scale_varlen_attn(torch::Tensor query,
                                                               torch::Tensor key,
                                                               torch::Tensor value,
                                                               torch::Tensor output,
            ...
            DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
                DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
                    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(out_dtype, DTypeOut, {
                        ...
                        SAGEATTN_CHECK_QK_SCALE_SHAPES_VARLEN(blk_total(total_q, batch_size, CTA_Q) * (CTA_Q / 32),
