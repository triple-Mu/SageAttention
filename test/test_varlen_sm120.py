"""The sm120 varlen kernel: the packed layout over the fp8 CUDA-core path.

Consumer Blackwell shares sm89's tile geometry and V^T slab (CTA_Q 128 /
CTA_K 64, pad 64) but its packed TU instantiates sm120's own default
pv_accum_dtype ("fp32", plan.cpp), so V quantizes over the full fp8 448 range
rather than sm89's fp16-accumulator headroom. The dense kernel sharing the
body is sm120::qk_int8_sv_f8_accum_f32_fuse_v_scale_attn, so on an
...
pytestmark = [requires_varlen, requires_backend("sm120")]

# sm120 quantization tile geometry (plan.cpp fill_tiles)
BLKQ, WARPQ, BLKK, WARPK = 128, 32, 64, 64
GEOM = dict(blk_q=BLKQ, warp_q=WARPQ, blk_k=BLKK, warp_k=WARPK)
V_PAD = 64  # plan.cpp v_pad_of(sm120); the launcher pins it equal to CTA_K
SCALE_MAX = 448.0  # plan.cpp v_scale_max: fp32 PV accumulation uses the full fp8 range
PV = "fp32"  # the only pv_accum_dtype the sm120 packed TU instantiates
...
def pack_v_fp8(v_packed, cu_k, max_seqlen_k):
    """The varlen V^T slab and its scales, as the sm120 kernel wants them:
