"""The sm90 varlen kernel: the packed layout over the fp8 V^T path.

test_varlen.py holds the sm80 half and the shared contract; this file is the
same set of gates on the arch whose value tensor is a block-padded transpose
and whose Q/K/V come in through tensor maps. The equal-length batch is still
the gate that pins the kernel: with delta == 0 the varlen tile structure is the
dense one, so the two must agree bit for bit.
...
pytestmark = [requires_varlen, requires_backend("sm90")]

# sm90 quantization tile geometry (plan.cpp fill_tiles) and V^T padding
BLKQ, WARPQ, BLKK, WARPK = 64, 16, 128, 128
GEOM = dict(blk_q=BLKQ, warp_q=WARPQ, blk_k=BLKK, warp_k=WARPK)
V_PAD = 128  # plan.cpp v_pad_of(sm90); fwd_cuda pins it equal to blk_k
SCALE_MAX = 448.0
PV = "fp32+fp32"  # the only pv_accum_dtype sm90 supports
...
def quant_v_dense(v_hnd):
    """[B, H, N, D] -> ([B, H, D, round_up(N, 128)] fp8, [B, H, D] scale)."""
...
def pack_v_fp8(v_packed, cu_k, max_seqlen_k):
    """The varlen V^T slab and its scales, as the sm90 kernel wants them:
    ``[kv_heads, head_dim, blk_total(total_k, batch, 128) * 128]`` fp8 and
...
def test_fwd_varlen_equals_dense(causal, gran, head_dim, b, h_qo, h_kv, n):
    ...
    the dense "mask only the epilogue" structure. Both outputs must therefore be
    bit-identical."""
...
def test_fwd_varlen_cudagraph():
    ...
    def attend():
        return OPS.fwd_varlen(
            ...
            pv_accum_dtype=PV, v_layout="mma_k16", is_causal=True,
